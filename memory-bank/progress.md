...
| **Wllama Worker (`@wllama/wllama`)** | Completed | WebGPU/WASM multi-threading, Gemma prompt template, token streaming, thought extraction, sequential downloads (`parallelDownloads: 1`), cache integrity checks, and duplicate INIT collision guards (`isLoading`/`isLoaded`). |
